...
func TestEncryptPlaintext(t *testing.T) {
  for key, text := range expectedEncryptionResults {
    ciphertext := caesar.EncryptPlaintext(testingString, key)
...
func TestDecryptCiphertext(t *testing.T) {
  for key, text := range expectedEncryptionResults {
    plaintext := caesar.DecryptCiphertext(text, key)
...
func ExampleDecryptCiphertext() {
  plaintext := caesar.DecryptCiphertext("Ebznaf, tb ubzr.", 13)
